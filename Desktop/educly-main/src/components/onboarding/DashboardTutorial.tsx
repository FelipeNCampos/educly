...
export const DashboardTutorial = () => {
  ...
  // Inicia o tutorial APENAS se o usuário estiver autenticado
  useEffect(() => {
    const checkAuthAndShowTutorial = async () => {
      const { data: { user } } = await supabase.auth.getUser();
      
      // Só mostra tutorial se usuário estiver logado
      if (!user) {
        return;
      }

      const hasSeen = localStorage.getItem("has_seen_dashboard_tutorial_v11");
      if (!hasSeen) {
  ...
  const handleClose = () => {
    setIsVisible(false);
    localStorage.setItem("has_seen_dashboard_tutorial_v11", "true");
  };
