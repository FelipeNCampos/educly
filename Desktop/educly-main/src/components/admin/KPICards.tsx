...
export const KPICards = () => {
  ...
  const { data: kpis, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const avgSessionMinutes = sessionData?.length
        ? (sessionData.reduce((acc, session) => {
            const start = new Date(session.started_at).getTime();
            const end = new Date(session.last_ping_at).getTime();
            return acc + (end - start);
          }, 0) / sessionData.length / 60000).toFixed(1)
        : 0;
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <AdminKPICard
            title="Total c/ Produto"
            value={kpis?.totalProductUsers || 0}
            icon={<Package className="h-5 w-5" />}
            description="usuários únicos"
            tooltip="Quantidade de usuários únicos com pelo menos 1 produto ativo em user_product_access"
          />
          <AdminKPICard
            title="Educly Base"
            value={kpis?.baseUsers || 0}
            icon={<Package className="h-5 w-5" />}
            description="product_type = 'base'"
            tooltip="Usuários únicos com product_type='base' e is_active=true em user_product_access"
          />
          <AdminKPICard
            title="Freelancer"
            value={kpis?.freelancerUsers || 0}
            icon={<Briefcase className="h-5 w-5" />}
            description="product_type = 'freelancer'"
            tooltip="Usuários únicos com product_type='freelancer' e is_active=true em user_product_access"
          />
          <AdminKPICard
            title="Hub de IAs"
            value={kpis?.aiHubUsers || 0}
            icon={<Bot className="h-5 w-5" />}
            description="product_type = 'ai_hub'"
            tooltip="Usuários únicos com product_type='ai_hub' e is_active=true em user_product_access"
          />
        </div>
      </section>
      ...
      <section>
        <AdminSectionHeader
          emoji="ℹ️"
          title="Métricas Especiais"
          description="Refunds e outros indicadores"
        />
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <AdminKPICard
            title="Refunds"
