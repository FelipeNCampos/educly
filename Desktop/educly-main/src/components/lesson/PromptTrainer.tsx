...
${userPrompt}`;
      ...
      if (resp.status === 401 || resp.status === 403) {
        throw new Error("Acesso não autorizado. Verifique sua assinatura.");
      }

      if (!resp.ok) {
        const errorData = await resp.json().catch(() => ({}));
        throw new Error(errorData.error || `Erro do servidor (${resp.status})`);
      }
      ...
      if (!fullContent || fullContent.trim().length < 10) {
        throw new Error("A IA não conseguiu gerar uma resposta. Tente reformular seu prompt.");
      }
      ...
      const errorMessage: Message = {
        role: "assistant",
        content: `⚠️ ${errorText}\n\nDica: Se o problema persistir, tente escrever um prompt mais curto e objetivo.`,
      };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {t("lesson.promptTrainer.subtitle", "Treine seu prompt com o EDI")}
            </p>
          </div>
        </div>
      </div>
