...
const DayLesson = () => {
  ...
  const [dayInfo, setDayInfo] = useState<{ dayNumber: number; title: string } | null>(null);
    ...
    const fetchDayContent = async () => {
      ...
      try {
        const { data: dayData } = await supabase
          ...
          .single();
        if (dayData) {
          setDayInfo({ dayNumber: dayData.day_number, title: dayData.title });
  ...
  const handleNext = async () => {
    ...
    // BLOQUEIO: Componentes interativos devem ser completados via callback
    if (step.type === "component" && step.componentName && getComponent(step.componentName) && !componentCompleted) {
    ...
    if (currentStepIndex >= lessonSteps.length - 1) {
      try {
        ...
        if (user && dayId && dayInfo) {
          ...
          await supabase.from("user_challenge_progress").update({ current_day: nextDayNumber }).eq("user_id", user.id);
        }
        navigate("/dashboard");
      } catch (error) {
        navigate("/dashboard");
  ...
  return (
    <div className="fixed inset-0 bg-background z-50 flex flex-col" onContextMenu={handleContextMenu}>
      ...
      {/* BOTTOM ACTION BAR - Hide for interactive components */}
      {!(currentStep.type === "component" && currentStep.componentName && getComponent(currentStep.componentName) && !componentCompleted) && (
          ...
          <div className="max-w-2xl mx-auto">
            <Button
              ...
              disabled={
                ...
                // Note: We removed the disabled condition for wrong answers so the "Try Again" button works
              }
              className={cn(
