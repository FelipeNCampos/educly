...
async function sendEmailViaSMTP(to: string, subject: string, html: string): Promise<void> {
  ...
  console.log(`[SMTP] Sending welcome email to ${to}`);
  console.log(`[SMTP] Using host: ${smtpHost}:${smtpPort}`);
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[send-welcome-email] Request received for ${email}, user: ${userName}, lang: ${language}`);
